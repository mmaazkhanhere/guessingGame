...
function turnOff(){ //function to turn off the animations
    return new Promise((resolve)=>{
        setTimeout(resolve,2500) //animations turn off after 2.5 secs
    })
}

async function display(){ //function for the animation
...
async function guessNumber(){ //function for guessing the number 
...
async function doAgain(){ //function to ask whether user want to play game again
    do{  //doing the following code at least once
        ...
        var repeat= await inquirer.prompt({ //user input his/her choice
            name:'userChoice', 
            type:'rawlist',
